...
class DynamicSprite extends Drawable{
  ...
  DynamicSprite(List<List<ImageElement>> t_frames, Location t_dimensions, {num t_framerate=4}) {
    frames = t_frames;
  ...
  void draw(CanvasRenderingContext2D ctx, Location loc) {
    for(ImageElement frame in frames[_curr_frame]) {
      ctx.drawImageScaled(
          frame, (loc.x - dimensions.x + 1) * TILE_SIZE,
          (loc.y - dimensions.y + 1) * TILE_SIZE,
